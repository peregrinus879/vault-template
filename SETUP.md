...
## Appendix: obsidian.nvim configuration (reference)

**This is a reference snapshot, not the live config.** Source of truth lives in the dotfiles repos:

- `~/projects/repos/dotfiles/dotfiles-arch/nvim/.config/nvim/lua/plugins/obsidian.lua`
- `~/projects/repos/dotfiles/dotfiles-omarchy/nvim/.config/nvim/lua/plugins/obsidian.lua`

Edit there, not here. This copy may drift; treat it as "what the vault expected at the time of this commit" rather than "what obsidian.nvim currently runs with." Forks cloning `vault-template` can use this snapshot as a starting point for their own dotfiles.

Captured against vault commit `fc8b9a9` (2026-04-16). Both dotfiles repos carry identical content; if they ever diverge, resolve from the machine currently in use.
